...
class Solution:  # pylint: disable=too-few-public-methods
    """Berechnet die minimale Anzahl benötigter Inkremente."""

    def minOperations(self, nums):  # pylint: disable=invalid-name
        """
        Gibt die minimale Anzahl von Operationen zurück.

        Gerade Indizes müssen Primzahlen enthalten.
        Ungerade Indizes müssen nicht-prime Zahlen enthalten.
        ...
        for index, number in enumerate(nums):
            if index % 2 == 0:
                ...
                operations += self._steps_to_non_prime(number, is_prime)

        return operations

    @staticmethod
    def _create_prime_table(limit):
        """Erstellt eine Tabelle für den Primstatus jeder Zahl."""
    ...
    def _create_next_prime_table(is_prime):
        """Speichert für jede Zahl die nächste Primzahl."""
    ...
    @staticmethod
    def _steps_to_non_prime(number, is_prime):
        """Berechnet die Schritte bis zur nächsten nicht-primen Zahl."""
        candidate = number
