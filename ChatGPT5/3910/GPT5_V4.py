...
class Solution:
    ...
    def evenSumSubgraphs(self, nums, edges):  # pylint: disable=invalid-name
        ...
        # Check every possible non-empty subset of nodes.
    ...
    def _has_even_sum(subset, nums):
        ...
        # XOR determines whether the sum is even or odd.
        while remaining:
    ...
    def _is_connected(subset, graph):
        ...
        # Start the traversal with any node in the subset.
        start = subset & -subset
        visited = start
        stack = start

        # Visit only nodes that belong to the current subset.
        while stack:
