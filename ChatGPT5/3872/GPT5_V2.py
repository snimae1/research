...
def _build_left_info(nums):
    """Build lengths and differences of arithmetic runs from the left."""
    n = len(nums)
    lengths = [1] * n
    differences = [None] * n
...
def _build_right_info(nums):
    """Build lengths and differences of arithmetic runs from the right."""
    n = len(nums)
    lengths = [1] * n
    differences = [None] * n
...
def _extend_left(lengths, index):
    """Replace nums[index] and extend the arithmetic run on the left."""
    return lengths[index - 1] + 1

@staticmethod
def _extend_right(lengths, index):
    """Replace nums[index] and extend the arithmetic run on the right."""
    return lengths[index + 1] + 1
...
def _bridge(
    ...
    The two surrounding elements determine the required common
    difference. It must be an integer and must match both existing
    arithmetic runs.
    """
    ...
    return left_length[index - 1] + 1 + right_length[index + 1]
```
