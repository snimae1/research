...
class Solution(object):
    ...
    def _solve_pattern(nums, first_parity):
        ...
        for index, value in enumerate(nums):
            ...
            if value % 2 == required_parity:
                # The value already has the required parity.
                intervals.append((value, value))
            else:
                # One operation can change the parity in either direction.
                operation_count += 1
    ...
    def _find_minimum_range(intervals):
        """Find the smallest range containing a valid value from every set."""
    ...
    def _range_is_possible(intervals, range_size):
        """Check whether all intervals can fit inside one range."""
        possible_left = None
        possible_right = None
